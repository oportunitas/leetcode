# [3925. Concatenate Array With Reverse][link] (Easy)

[link]: https://leetcode.com/problems/concatenate-array-with-reverse/

You are given an integer array `nums` of length `n`.

Construct a new array `ans` of length `2 * n` such that the first `n` elements are the same as
`nums`, and the next `n` elements are the elements of `nums` in reverse order.
