# [2037. Minimum Number of Moves to Seat Everyone][link] (Easy)
...
`seats` of length `n`, where `seats[i]` is the position of the `iᵗʰ` seat. You are also given the
array `students` of length `n`, where `students[j]` is the position of the `jᵗʰ` student.
