# [2161. Partition Array According to Given Pivot][link] (Medium)
...
`pⱼ` is the new position of the `jᵗʰ` element. If `i < j` and **both** elements are smaller (or
larger) than `pivot`, then `pᵢ < pⱼ`.
